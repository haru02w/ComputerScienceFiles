...
bp_node *bp_next(bp_tree *tree, bp_node *node)
{
	if (node->is_leaf == false) {
		printf("NOT LEAF\n");
		exit(EXIT_FAILURE);
	}
	if (node->next_offset == -1)
		return NULL;
	ibp_node_read(node, tree, node->next_offset);
	return node;
}

bp_node *ibp_search_node(bp_tree *tree, const char *key)
{
...
int bp_search_data_rrn(bp_tree *tree, const char *key)
{
...
void bp_insert(bp_tree *tree, const char *key, const int data_rrn)
{
